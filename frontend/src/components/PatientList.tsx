...
const PatientList: React.FC = () => {
  ...
  const filteredPatients = selectedCategory === 'All' ? displayPatients : displayPatients.filter(patient => patient.category === selectedCategory)
    ...
    <div className='dashboard-card'>
      <div className='flex items-center justify-between mb-6'>
        ...
        <div className='flex items-center gap-4'>
                ...
                <div className='py-1'>
                  {categories.map(category => (
                    <Menu.Item key={category}>
                      {({ active }) => (
                        <button onClick={() => setSelectedCategory(category)} className={`${active ? 'bg-indigo-500/20' : ''} ${selectedCategory === category ? 'text-indigo-400' : 'text-gray-200'} flex w-full px-4 py-2 text-sm`}>
                          {category}
                        </button>
                      )}
                    </Menu.Item>
          ...
          <button onClick={() => setShowAll(!showAll)} className='px-4 py-2 bg-indigo-500/10 rounded-lg text-sm font-medium text-indigo-400 hover:bg-indigo-500/20 transition-colors'>
                ...
                <tr key={patient.id} className='border-t border-indigo-500/10 hover:bg-indigo-500/5 transition-colors'>
                  ...
                  <td className='py-4'>
                    <div className='flex items-center'>
                      <img src={patient.avatar} alt={patient.name} className='w-8 h-8 rounded-full mr-3 ring-2 ring-indigo-500/20' />
                      <span className='text-gray-200'>{patient.name}</span>
                    </div>
                  </td>
                  <td className='py-4 text-gray-300'>{patient.age} years</td>
                  <td className='py-4'>
                    <span className={`px-3 py-1 rounded-full text-sm ${patient.category === 'Complex' ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300'}`}>{patient.category}</span>
                  </td>
                  <td className='py-4'>
                    <div className='flex flex-wrap gap-2'>
                      {patient.conditions.map((condition, index) => (
                        <span key={index} className='px-2 py-1 rounded-full text-xs bg-indigo-500/20 text-indigo-300'>
                          {condition}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className='py-4'>
                    <span className='px-3 py-1 rounded-full text-sm bg-emerald-500/20 text-emerald-300'>{patient.status}</span>
                  </td>
